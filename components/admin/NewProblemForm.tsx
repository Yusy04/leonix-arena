  ...
  return (
    <div className="adm container-narrow">
      ...
      <form className="adm-card stack-4" onSubmit={submit}>
        ...
        <div className="field"><label>Original language</label><input className="input mono" value={f.originalLanguage} onChange={e => setF({ ...f, originalLanguage: e.target.value })} placeholder="ro"/>{errors.originalLanguage && <span className="field-error">{errors.originalLanguage}</span>}<span className="t-xs dim" style={{ marginTop: 6, display: "block" }}>Just the language code your statement will be written in — e.g. <code>ro</code> or <code>en</code>. You&apos;ll write the statement itself (with formatting and inline images) in the next step.</span></div>
